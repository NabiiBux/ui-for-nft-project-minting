...
export const guardChecker = async (
  ...
  if (
    ...
    for (const eachGuard of guardsToCheck) {
      // console.log("data data", candyMachine)
      guardReturn.push({
  ...
  if (
    ...
    for (const eachGuard of guardsToCheck) {
      guardReturn.push({
        label: eachGuard.label,
        allowed: false ,
        reason: "All NFTs have been minted",
      });
    }
    return { guardReturn, ownedNfts: ownedTokens };
  }
  ...
  for (const eachGuard of guardsToCheck) {
    ...
    if (singleGuard.redeemedAmount.__option === "Some" &&     Number(candyMachine.data.itemsAvailable) - Number(candyMachine.itemsRedeemed) === 0
    ) {
      ...
      if (redeemedAmount.value.maximum >= candyMachine.itemsRedeemed) {
        guardReturn.push({
          label: eachGuard.label,
          allowed: false,
          reason: "All available NFTs redeemed!",
        });
        console.info(
    ...
    if (singleGuard.startDate.__option === "Some") {
      ...
      if (solanaTime < startDate.value.date) {
        guardReturn.push({
          label: eachGuard.label,
          allowed: false,
          reason: "Mint not live yet",
        });
        console.info(`${eachGuard.label} Mint not live yet`);
